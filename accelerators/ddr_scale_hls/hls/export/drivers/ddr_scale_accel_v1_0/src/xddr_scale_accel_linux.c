...
typedef struct {
    ...
    XDdr_scale_accel_uio_map maps[ MAX_UIO_MAPS ];
} XDdr_scale_accel_uio_info;

/***************** Variable Definitions **************************************/
static XDdr_scale_accel_uio_info uio_info;
...
static int uio_info_read_name(XDdr_scale_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDdr_scale_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDdr_scale_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDdr_scale_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDdr_scale_accel_Initialize(XDdr_scale_accel *InstancePtr, const char* InstanceName) {
	XDdr_scale_accel_uio_info *InfoPtr = &uio_info;
